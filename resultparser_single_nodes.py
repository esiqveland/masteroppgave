...
def main():
    ...
    print "files to parse:", paths

    if len(paths) == 0:
        print "no input files found"
        sys.exit(-1)
    
    operations = {'reads': {}, 'writes': {}, 'updates': {}}
...
#                if "iteration = 0" not in line:
            ...
            for line in myfile:
                ...
                if result:
                    ...
                    myoperationdict = operations[operation]
                    if responsetime not in myoperationdict:
                        myoperationdict[responsetime] = 0
                        myoperationdict[responsetime] = myoperationdict[responsetime] + count

    for myoperationdict in operations.values():
        for responsetime, count in myoperationdict.iteritems():
            if responsetime not in histogram:
                histogram[responsetime] = 0
            histogram[responsetime] += count

    print "\ntotal operations tracked:", sum(histogram.values())
    #print histogram
