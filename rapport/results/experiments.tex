...
In this test we will move a few partitions in a very large dataset.

We will insert 100 million records of 1024, then move partitions.

This is so we can hopefully record base throughput before, during, and after the data is actually moved over a longer period of time.
This is of interest because the other tests prove quite short when moving data, and is not the best for comparison, when trying to find the performance cost of moving.
