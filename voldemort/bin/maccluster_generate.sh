...
CONFIG_DIR="${PROJECT_DIR}/config/maccluster/config"
...
python2.7 generate_cluster_xml.py  \
    --file maccluster.hostnames.txt --name picluster  \
    --nodes ${NUM_NODES} --partitions ${PARTITIONS}  \
    --socket-port 6666 --http-port 8081 --rest-port 8085 --admin-port 6667 \
